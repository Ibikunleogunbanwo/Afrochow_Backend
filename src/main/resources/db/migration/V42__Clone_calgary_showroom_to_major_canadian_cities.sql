...
-- Description: Clone the trusted Calgary showroom flow to major Canadian cities.
--
-- This migration copies the existing production Calgary seed vendors, products,
...
DROP TEMPORARY TABLE IF EXISTS major_market_specs;

CREATE TEMPORARY TABLE major_market_specs (
...
DROP TEMPORARY TABLE IF EXISTS major_seed_vendors_to_remove;
DROP TEMPORARY TABLE IF EXISTS major_seed_vendor_users_to_remove;
DROP TEMPORARY TABLE IF EXISTS major_seed_reviewers_to_remove;
DROP TEMPORARY TABLE IF EXISTS major_seed_products_to_remove;
DROP TEMPORARY TABLE IF EXISTS major_seed_addresses_to_remove;

CREATE TEMPORARY TABLE major_seed_vendor_users_to_remove AS
SELECT u.user_id
FROM users u
...
CREATE TEMPORARY TABLE major_seed_vendors_to_remove AS
...
CREATE TEMPORARY TABLE major_seed_reviewers_to_remove AS
SELECT u.user_id
FROM users u
JOIN major_market_specs ms
...
CREATE TEMPORARY TABLE major_seed_products_to_remove AS
SELECT p.product_id
FROM product p
...
CREATE TEMPORARY TABLE major_seed_addresses_to_remove AS
SELECT a.address_id
FROM address a
JOIN major_market_specs ms
...
DROP TEMPORARY TABLE IF EXISTS major_source_vendors;

CREATE TEMPORARY TABLE major_source_vendors AS
SELECT ranked.*
FROM (
    SELECT
...
DROP TEMPORARY TABLE IF EXISTS major_vendor_slots;

CREATE TEMPORARY TABLE major_vendor_slots AS
SELECT
    ms.market_code,
    ms.city,
    ms.province,
...
DROP TEMPORARY TABLE IF EXISTS major_vendor_map;

CREATE TEMPORARY TABLE major_vendor_map AS
SELECT
    vs.market_code,
    vs.slot_no,
...
DROP TEMPORARY TABLE IF EXISTS major_source_products;

CREATE TEMPORARY TABLE major_source_products AS
SELECT
    product_ranked.*,
...
DROP TEMPORARY TABLE IF EXISTS major_product_map;

CREATE TEMPORARY TABLE major_product_map AS
SELECT
    sp.market_code,
...
DROP TEMPORARY TABLE IF EXISTS major_source_reviews;

CREATE TEMPORARY TABLE major_source_reviews AS
SELECT
    sr_ranked.*
FROM (
    SELECT
